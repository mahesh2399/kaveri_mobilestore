...
class SelectedCategoryBloc extends Bloc<SelectedCategoryEvent, SelectedCategoryState> {
  ...
  SelectedCategoryBloc(this.selectedCategoryService) : super(SelectedCategoryInitial()) {
    on<FetchSelectedCategoryEvent>((event, emit) async {
      emit(SelectedCategoryLoading());
      try {
        final List<Category> categories = await selectedCategoryService.fetchCategories(event.categoryId);
